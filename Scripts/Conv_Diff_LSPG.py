...
A2D_conv=sp.kron(A1D_conv,sp.eye(N-1,format='csc'),format='csc')+sp.kron(sp.eye(N-1,format='csc'),A1D_conv,format='csc')

# snapshot, free DoFs, No IC included
U=np.zeros((Ns,no_para*Nt))
for p in range(no_para):
    A=-mu1[p]*A2D_conv + mu2[p]*A2D_diff; invA=splu(I-k*A); u=np.zeros((Ns,Nt+1)); u[:,0]=u0
    for n in range(Nt):
...
UROM_pg2=np.zeros((num_test,Nt+1,N+1,N+1)); UFOM_2=np.zeros((num_test,Nt+1,N+1,N+1))
...
for i in range(Nt):
    for j in range(nt):
        PHIstij=np.zeros((Ns,ns)); Dij=sp.diags(D[:,j*Nt+i],format='csc')
...
DPHIsTu0=np.zeros(ns*nt); DA1sTu0=np.zeros(ns*nt); DA2sTu0=np.zeros(ns*nt)
us0=PHIsT@u0; PHIsTA1Tu0=PHIsTA1T@u0; PHIsTA2Tu0=PHIsTA2T@u0
for j in range(nt):
    Djk=sp.diags(D[:,j*Nt],format='csc')
    DPHIsTu0[ns*j:ns*(j+1)]=Djk@us0; DA1sTu0[ns*j:ns*(j+1)]=-k*Djk@PHIsTA1Tu0; DA2sTu0[ns*j:ns*(j+1)]=-k*Djk@PHIsTA2Tu0

# predictive cases
for test_count in range(num_test):
    param1,param2=ParamD1[test_count],ParamD2[test_count] # set parameter mu

    # construct Ast, ust (fst is zero)
    Ast=DD-param1*DA1sD+param2*DA2sD-param1*param2*DA1sA2sD+param1**2*DA1sA1sD+param2**2*DA2sA2sD
    ust=DPHIsTu0-param1*DA1sTu0+param2*DA2sTu0  
    ...
    UROM_pg2[test_count]=urom; UFOM_2[test_count]=ufom